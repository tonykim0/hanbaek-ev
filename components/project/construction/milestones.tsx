...
export function groupsByStatus(
  ...
  return {
    ...
    '준공서류 접수/검토': [
      {
        ...
        docs: [
          ...
          'safety',
          /*
           * ★사용전점검/검사 필증 바로 밑이다★ (한백 2026-09-06) — 같은 전기안전점검에서 나오는
           * 둘이라 나란히 서야 한 눈에 읽힌다(필증과 그 수수료 영수증).
           *
           * ★완료 선언의 조건은 아니다★ — 준공 「뒤에」 오는 서류라 lib/process 의
           * COMPLETION_DOCS 에 넣지 않았다. 넣으면 영수증이 없어 준공을 못 끝내고,
           * 준공을 못 끝내서 영수증이 안 오는 교착이 된다(옛 「준공서류」 한 칸이 그 꼴이었다).
           * 받는 운영사가 아니면 칸이 안 선다 — 조건은 doc-rules 의 only 가 본다.
           */
          'safetyFeeReceipt',
          'safetyMgr',
          'useInspect',
    ...
    /*
     * ★준공완료 구간에도 자리가 있어야 한다★ (2026-09-08).
     *
     * 전기안전점검수수료 영수증은 ★준공완료 뒤에★ 협력사에게서 받는 서류다(한백 2026-09-06).
     * 그런데 이 표에 '준공완료' 키가 없어서, 그 구간에 들어간 현장의 시공 탭은 제목과
     * 「지금 구간」 배지만 있는 빈 패널이었다 — 낼 때가 된 순간 낼 자리가 사라진다.
     * 협력사는 「지난 구간」 칩을 눌러 준공서류 접수/검토로 되돌아가야 그 칸을 봤다.
     *
     * 그래서 마지막 구간에 그 칸 하나를 세운다. 준공 조건 서류는 여기 적지 않는다 —
     * 그것들은 이미 낸 뒤라 이 구간의 일이 아니고, 적으면 끝난 서류를 다시 묻는 자리가 된다.
     * 받는 운영사가 아니면 칸이 안 선다(doc-rules 의 only) — 그때는 이 상자가 빈 채로
     * 서지만, 그것이 「이 현장에는 낼 것이 없다」는 사실이다.
     */
    '준공완료': [
      {
        title: '준공 후 정산 서류',
