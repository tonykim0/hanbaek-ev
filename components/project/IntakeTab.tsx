...
function SiteFacts(
  ...
  const biz: FactRow[] = [
    /*
     * 여기서는 읽기만 한다 — 같은 값을 한 화면에서 두 자리로 고치지 않는다(화면 규칙 5).
     *   현장관리번호 — 접수 번호 그 자체다. 어디서도 안 고친다.
     *   사업구분·수전방식 — ★머리말에서 고친다★ (한백 지시 2026-09-10). 단가 케이스의
     *     축이라 잠금·단가 해제가 붙고, 사업구분은 흐름까지 바꿔서 지금 서 있는 칸이 새
     *     흐름에 있는지도 본다(저장소 setProjectAxes). 그 판정을 한 자리에 모아 두려고
     *     머리말 하나만 고치는 자리를 갖는다.
     *   교체유형 — ★아직 고치는 자리가 없다.★ 같은 축인데 머리말에 안 서 있어서다.
     *     운영사를 고치면 여기도 따라 눕는다(normalizeRepl) — 그것만 자동이다.
     *   계약서 수령일·접수·확인 — 사람이 한 일의 기록이다. 각자 단추가 따로 있다.
     */
...
export function IntakeTab({
   ...
   *
   * ★계약 국면의 두 칸에서 민다 — 계약완료와 운영사 계약서 제출★ (한백 지시 2026-09-22
   * 「운영사 계약서 제출 단계에 있는 현장에 행위신고로 넘기기 버튼 추가해줘」).
   *
   * 그 둘은 시공 스테퍼가 그리지 않는 칸이다(ConstructionTab 은 행위신고부터 그린다) —
   * 그래서 「두 자리에서 민다」가 아니다. 운영사가 승인을 알려 오면 그 소식을 듣는 사람은
   * 계약 보드를 보고 있는데, 앞으로 가려면 시공 탭을 따로 열어야 했다. 그 앞(접수·검토)은
   * 확인이 먼저라 여기 없고, 행위신고부터는 시공 탭 스테퍼가 민다(화면 규칙 5).
  ...
  const nextStep = status === '계약완료' || status === HANDOFF_STATUS
    ? nextStatusOf(status, gate)
    : null;
   ...
   *
   * 「운영사 계약서 제출」일 때만이다 — 그 칸에서는 미는 단추(행위신고 로 넘기기)와 나란히
   * 선다. 행위신고부터는 시공 탭 스테퍼가 밀고 되돌린다(화면 규칙 5).
   */
      ...
      <section>
        ...
        */}
        ...
        {canSubmit
           ...
           * 계약보완 칸에 서 있고, 협력사가 요청을 눌러야 검토로 온다(lib/board).
           *
