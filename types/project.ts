 ...
 */
/*
 * ★SK일렉링크는 2026-09-01 정책부터 가르지 않는다★ (한백 지시 2026-09-10 「자체투자는 이제
 * 제자리교체와 신규위치 구분없이 7년 10년 각각 220, 230」). 7/20 케이스에는 신규위치 행이
 * 남아 있고 그 라인들은 그 케이스를 그대로 참조한다(후보 판정은 라인의 교체유형을 보므로
 * 옛 라인은 옛 케이스에 그대로 붙는다). 새 접수는 「자체투자」 한 칸 — 대표값 제자리교체다.
 */
export const SPLITS_SELF_REPL: ReadonlySet<CpoName> = new Set(['에버온']);
