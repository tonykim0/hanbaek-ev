 ...
 */
export type ReplType = '환경부 신규' | '자체투자 (제자리교체)' | '자체투자 (신규위치)' | '기설치 연동';
export const REPL_TYPES = [
  '환경부 신규', '자체투자 (제자리교체)', '자체투자 (신규위치)', '기설치 연동',
...
/*
 * ★「연동」을 「기설치 연동」으로 부른다★ (한백 지시 2026-09-07) — 이미 깔려 있는
 * 충전기를 운영사 시스템에 붙이는 사업이라, 그냥 「연동」이면 무엇을 연동하는지가 없다.
 * 프로덕션에 이 구분의 현장이 아직 0건일 때 갈았다(단가 케이스 6줄만 옮겼다 —
...
export function bizTypeOfRepl(repl: ReplType): BizType {
  return repl === '환경부 신규' ? '환경부' : repl === '기설치 연동' ? '기설치 연동' : '자체투자';
}
 ...
 */
/*
 * ★SK일렉링크는 다시 가른다★ (한백 지시 2026-09-17 「신규설치냐 제자리교체냐 따로 지정해야해 ·
 * 9월 1일 이전에는 이게 나뉘어져 있어서 갈라야해」).
 *
 * 2026-09-10 에 뺐었다 — 9/1 정책이 「구분없이 220·230」이라서다. 그런데 이 집합은 ★운영사★만
 * 보고 ★시기★를 못 본다. 빼는 순간 9/1 이전 계약분까지 같이 눕었다: 7/20~8/31 은 제자리교체
 * 150 과 신규위치 200·210 이 실제로 갈리는데, 새로 들어오는 자체투자 라인이 전부 제자리교체로
 * 눕어 50~60만이 조용히 낮게 붙었다(한백이 잡았다). 이미 붙은 라인은 무사했다 — 매칭은 라인의
 * 교체유형을 보고, 프로덕션에 신규위치 라인 11개가 그대로 남아 있었다.
 *
 * 그래서 축은 늘 세워 두고 ★값이 같은 시기는 케이스를 두 벌 둔다★ — 9/1 자투는 제자리교체와
 * 신규위치가 같은 220·230 이다(migrations/0079). 고르는 사람은 사실대로 고르고, 9/1 뒤에는
 * 어느 쪽을 골라도 금액이 같다. 시기를 아는 축으로 바꾸는 것(replTypesOf 에 날짜를 넘기는 것)이
 * 더 옳지만, 부르는 자리가 열 곳이 넘고 그중 접수 폼은 계약일을 아직 모른다.
 */
/*
 * ★나머지 셋은 구분이 없다 — 한백 확인 2026-09-22★
 * 「플러그링크·나이스·현대엔지니어링의 자체투자에서는 제자리교체랑 위치로 옮겨서 설치하는거
 *  구분이 없어」. 단가표도 그렇다: 그 셋의 활성 자체투자 케이스는 제자리교체뿐이고
 *  신규위치는 0 개다(프로덕션 실측 2026-09-22 — 플러그링크 4 · 나이스 2 · 현대 1).
 *
 * ★이 집합을 고치기 전에 이 문단을 읽는다.★ 운영사를 넣고 빼는 것이 곧 돈이다 — 뺀 운영사의
 * 새 라인은 전부 제자리교체로 눕고, 그 운영사가 위치로 단가를 가르면 그 차액만큼 조용히 낮게
 * 붙는다(2026-09-10 → 09-17 에 SK 로 실제로 겪었다: 50~60만). 넣고 빼는 근거는 ★그 운영사의
 * 정책표★이지 이번 시기 금액이 같다는 사실이 아니다.
 */
...
export interface ProcessInfo {
  ...
  notifyDoneAt: string | null;       // 행위신고 완료 → 「충전기 발주」 조건 (연동은 「개통 및 통신확인」)
  /**
...
export interface PayoutPlanRow {
  ...
  holdState: HoldState | null;
  /**
   * ★받은 것을 그대로 내려주는 현장인가★ (한백 지시 2026-09-22 · 화두에너지솔루션).
   *
   * 그런 현장은 「1차 70% · 2차 잔액」으로 나가지 않는다 — 「1차 70% / 2차 잔액 대로는
   * 지급 안 할거야」(한백 2026-09-23). 시점은 나중에 정하기로 했으므로, 지금 화면에 회차를
   * 세우면 나가지도 않을 금액을 적는 것이고 ★체크 한 번에 70% 가 실제로 나간다.★
   * 그래서 이 줄은 회차를 접고 총 지급액만 보여준다.
   */
  passThrough: boolean;
...
export interface SettlementSummary {
   ...
   * ★받은 것을 그대로 내려주는 현장인가★ (한백 지시 2026-09-22 · 화두에너지솔루션).
   *
   * 한백 몫에서 뺀다 — 「패스스루건은 따로 우리 마진이 없기 때문에 한백 마진에서 제외하자」
   * (한백 2026-09-23). 지금은 셈만으로도 0 이 되지만(기성 − 영업비 − 시공비, 계획이 턴키다)
   * 그것은 우연히 맞는 것이다: 협의로 계획과 다르게 받으면 그 차액이 한백 마진으로 새어
   * 들어간다. 그 돈도 그 회사 것이므로 현장째 빼는 것이 사실에 맞다.
   */
  passThrough: boolean;
  /** 영업비 계획 = Σ(영업비/대 × 대수) */
  salesTotal: number;
 ...
 */
/**
 * 진행현황이 서는 자리 — ★탭마다 자기 이야기를 갖는다★ (한백 지시 2026-09-17·09-18).
 * 머리말 아래 한 자리에 다 쌓일 때는 계약 때 오간 말과 시공 중의 사정이 한 줄기로 섞여서,
 * 한쪽을 보는 사람이 다른 쪽을 걷어내며 읽어야 했다.
 *
 * 「기성」은 ★한백만 보고 한백만 쓴다★ — 운영사에게서 받는 돈 이야기라 협력사에게는 그 탭
 * 자체가 없다. 협력사 응답에서는 그 갈래를 빼고(redactForViewer), 쓰는 문도 한백에게만 연다
 * (api/projects/[id]/notes). 협력사 정산관리 탭은 이 갈래를 쓰지 않는다 — 거기는 원래 있던
 * 지급 메모(settlements.pay_note)가 그 자리를 맡는다(한백 지시 2026-09-18 「기존에있던 메모를
 * 제목만 바꿔서 이동」).
 */
export const NOTE_SCOPES = ['계약', '시공', '기성'] as const;
 ...
 * (협력사 서류 · 안전관리비 영수증 · 공지 첨부) — 한 곳에 모은다. ★화면도 이 값을 본다★:
 * 화면이 모르면 4.2MB 짜리 휴대폰 사진을 올려 보고 나서야 튕긴다.
 *
 * 넘으면 막지 않고 ★그 자리에서 사진을 다시 구워 올린다★ (한백 지시 2026-09-05 —
 * 제일전기통신 사업자등록증·통장사본이 4.2MB 로 막혔다). 서류 칸이 100MB 에서 하는 일과
 * 같은 길이다(lib/shrink) — 휴대폰 사진 한 장은 1600px·q72 로 구우면 대개 수백 KB 가 된다.
 */
