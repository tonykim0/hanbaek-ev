...
export const PROCESS_STATUSES = [
  ...
  '운영사 계약서 제출',
  /*
   * ★기설치 연동만 지나는 두 칸★ (한백 지시 2026-09-07).
   *
   * 이미 깔린 충전기를 운영사 시스템에 붙이는 사업이라 발주·수령·착공이 없다. 대신
   * 계약 뒤에 전기사용신청 → 전기안전점검 → 충전시설신고(= 행위신고와 같은 신고)를
   * 거친다. 그 셋 중 신고는 이미 있는 칸이라 새로 만드는 것은 앞의 둘뿐이다.
   *
   * ★칸 목록은 하나로 둔다★ — 사업구분마다 목록을 따로 두면 보드·할 일·게이트가
   * 두 벌이 된다. 대신 현장마다 ★지나는 칸의 부분집합★을 고른다(stepsOf) — 두 흐름 다
   * 이 순서대로 올라가므로 한 줄기로 표현된다:
   *
   *   환경부·자체투자  … 운영사 계약서 제출 → 행위신고 → 발주 → 수령 → 착공 → 개통 …
   *   기설치 연동      … 운영사 계약서 제출 → 전기사용신청 → 전기안전점검 → 행위신고 → 개통 …
   */
  '전기사용신청',
  '전기안전점검',
  /*
...
export interface ProcessInfo {
  ...
  elecApplyDate: string | null;
  elecApplyDoneAt: string | null;    // 전기사용신청 완료 → 「전기안전점검」이 열린다
  safetyCheckDate: string | null;
  safetyCheckDoneAt: string | null;  // 전기안전점검 완료 → 「행위신고」가 열린다
