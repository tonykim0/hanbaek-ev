 ...
 * 준공서류 칸의 사용전점검/검사 필증이 그것이다. 절차를 칸으로 새로 만들었다가 걷었다 —
...
/**
 * 선행조건이 안 찼는데도 그 선언을 찍을 수 있는가 (한백 지시 2026-09-14
 * 「이거 없이 한백이 강제로 버튼을 누를 수 있게 해줘」).
 *
 * ★준공서류 제출 완료 하나뿐이고, 한백만이다.★
 *   그 선언은 ★단계를 옮기지 않는다★ — 「준공서류 접수/검토」에 서서 「다 받았다」고 말해
 *   한백 자신의 검토 판정(CompletionReview)을 여는 자리다(CHECK_AT 에 이 값이 없는 까닭).
 *   종이 한 장이 늦게 오는 현장에서 한백이 자기 판단으로 검토를 시작할 수 있어야 한다.
 *
 * ★나머지는 그대로 막는다.★ 설치완료·개통완료 선언은 지급 트리거라(payoutReleaseOf)
 * 강제로 찍으면 사진도 착공일도 없이 돈이 열린다 — 감사 2026-09-04 H2 가 그 자리다.
 * ★준공완료로 넘어가는 문도 그대로다★ — 거기서 같은 서류를 다시 묻는다(canEnter).
 * 2차 지급 30% 가 열리는 문이라 이 강제가 거기까지 가지 않는다.
 *
 * ★시공사에게는 주지 않는다★ — 안 내고 「냈다」고 말할 수 있으면 그 선언이 뜻을 잃는다.
 * 판정을 여기 두는 이유는 화면·서버가 같은 것을 봐야 하고, 시험이 닿아야 해서다.
 */
export function mayForceDeclaration(field: string, role: Role): boolean {
  return field === 'completionSubmitAt' && role === 'admin';
}
