...
export function toProcess(projectId: string, r: ProcRow | undefined, docRows: ProcDocRow[]): ProcessInfo {
  return {
    ...
    elecApplyDate: r?.elecApplyDate ?? null,
    elecApplyDoneAt: r?.elecApplyDoneAt ?? null,
    safetyCheckDate: r?.safetyCheckDate ?? null,
    safetyCheckDoneAt: r?.safetyCheckDoneAt ?? null,
...
export function toSettlementRaw(projectId: string, r: SettlementRow | undefined): Omit<Settlement, 'steps'> {
  return {
    ...
    safetyFeeReceipts: r?.safetyFeeReceipts ?? [],
