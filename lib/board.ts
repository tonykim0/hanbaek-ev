...
const PARTNER_WAITING: Partial<Record<ProcessStatus, string>> = {
  ...
  /*
   * ★「준공서류 접수/검토」 항목을 지웠다★ (2026-09-08 검증) — 그 칸을 여는 것은
   * 시공사가 개통 상자에서 누르는 「다음 단계로 진행」(openDoneAt)이다. 한백 일이 아닌데
   * 「한백 검토 대기 중」이라 적어서, 개통 및 통신확인에 선 시공사가 자기 일(통신완료일·
   * 개통 완료 선언)을 앞에 두고 기다리라는 말을 읽었다.
   */
  '준공완료': '한백 준공마감 대기 중',
};
