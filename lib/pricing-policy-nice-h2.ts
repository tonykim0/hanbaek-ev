 ...
 * 내부 사정이라, 표에 있으면 화면·캡처로 협력사에게 새 나간다(한백 확인 2026-08-23).
 * 그 사실의 정본은 safetyFeeBearer 값이다 — 화면에 안 나온다.
 *
 * ★기성금액에 포함된 돈이 아니다★ (한백 2026-09-06 「단가표에 있는 기성금액에다가
 * 전기안전점검수수료를 따로 받아야해」) — 예전에 이 자리에 적어 두었던 「(턴키금액 포함)」은
 * 사실이 아니었다. 실제 청구액은 현장마다 settlements.safety_fee 가 받는다.
 */
...
function ruleOf(row: PolicyRow): NewPricingRule {
  ...
  return {
    ...
    /*
     * ★프로덕션 값에 맞췄다 (2026-09-08 실측).★ 이 파일에 적혀 있던 「한백 수령 · 하도급
     * 미지급(턴키금액 포함)」은 프로덕션에 간 적이 없다 — 그 값을 넣은 커밋(2026-08-23)의
     * 마이그레이션에 이 칸이 없었고, 프로덕션 나이스 7행은 지금도 「한백 대납(회수)」다
     * (SK·현대엔지니어링도 같다). 그래서 0066 의 나이스 update 는 0행을 고쳤고 검산은
     * 「턴키금액 포함이 없다」만 보아 초록불을 줬다 — 애초에 없던 문구다.
     *
     * 「(턴키금액 포함)」이 사실이 아니라는 것은 그대로다(한백 2026-09-06 「단가표에 있는
     * 기성금액에다가 따로 받아야해」) — 대납하고 회수하는 것이 곧 따로 받는 것이라
     * 프로덕션 값이 사실에 맞다. 정본은 DB 이고 이 파일은 그것과 같은 말을 해야 한다.
     */
