...
function ruleOf(row: PolicyRow): NewPricingRule {
  ...
  return {
    ...
    /*
     * ★「(턴키금액 포함)」을 뗐다 (한백 2026-09-06).★ 사실이 아니다 — 「단가표에 있는
     * 기성금액에다가 전기안전점검수수료를 따로 받아야해」다. 포함이라고 적혀 있으면
     * 다음 사람이 이 수수료를 이중 계상으로 읽는다(실제로 그 의심이 검증에서 나왔다).
     * 남는 사실은 둘이다: 한백이 받고, 하도급사에는 안 준다. 반영은 migrations/0066.
     */
    safetyFeeBearer: '한백 수령 · 하도급 미지급 · 기성과 별도 청구',
    supplyItems: SUPPLY,
